...
const SYSTEM_INSTRUCTION = `You are the "Admissions Assistant" for the Spring School on Sports Science 2024.
...
Details:
- Dates: March 2nd - 6th, 2024.
- Location: IIT Delhi Main Campus.
- Tuition: INR 25,000 + GST.
- Curriculum: Biomechanics, Physiology, AI in Sports (Modules M101-C400).
- Accommodation: On-campus Guest House available.
- Certification: Provided upon completion of the Capstone project.
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: (Date.now() + 1).toString(),
        role: 'model',
        text: "I am encountering a technical issue. Please inquire again momentarily."
      }]);
    } finally {
      setIsLoading(false);
    }
              ...
              {messages.map((msg) => (
                <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={`flex flex-col gap-2 max-w-[85%] ${msg.role === 'user' ? 'items-end' : ''}`}>
                    <div className={`p-3.5 shadow-sm border ${
                      msg.role === 'user' 
